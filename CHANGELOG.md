...
- #149, adding new functionality to subjoin. Feature adds new bool flag '--avg_reps'; if provided, the input will expect the name list file provided with the input matrix to be a file containing at least 2 columns and 1 row. The first column of each row is the output column name for the averaged columns. Output column name (first column), the first sample column to include in average (second column), the second column to include in average (third column), and so on for the number of columns to average. eg. The average with three total columns provided: per row, column 2 score + column 3 score / 2 -> column 1.
## [1.4.0] - 2021-07-09
