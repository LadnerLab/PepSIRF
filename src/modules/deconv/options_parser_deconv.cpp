...
bool options_parser_deconv::parse( int argc, char ***argv, options *opts )
{
    ...
    desc.add_options()
        ...
        ( "fractional_scoring", po::bool_switch( &opts_deconv->fractional_scoring )->default_value( false ),
          "Include this option to use fractional instead of integer scoring (default). With integer scoring, "
          "each taxon receives one point for each peptide to which it is 'linked' (i.e., with which it shares a kmer). "
          "For fractional scoring, the score for each taxon is 1/n, where n is the number of taxa 'linked' "
          "to the peptide. In this method of scoring, peptides linked to fewer taxa (i.e., more specific) "
          "are given higher scores. For integer and fractional scoring, each line in the file passed to "
          "'--linked' should be of the form \"peptide_name TAB id,id\" and so on. Note that if neither this flag "
          "nor --summation_scoring are included, integer scoring will be used.\n"
        )
        ( "summation_scoring", po::bool_switch( &opts_deconv->summation_scoring )->default_value( false ),
          "Include this option to use summation scoring instead of fractional or integer scoring. For summation "
          "scoring, the '--linked' file passed must be of the form created by the 'link' module. This means a file of "
          "tab-delimited values, one per line. Each line is of the form \"peptide_name TAB id:score,id:score\", "
          "and so on. For summation scoring, the peptide score for each taxon is as specified in the '--linked' file, "
          "which, if generated by the 'link' module, will be weighted by the number of kmers shared between the "
          "peptide and proteins derived from the taxon. Note that if neither this flag nor --summation_scoring "
          "are included, integer scoring will be used.\n"
        )
