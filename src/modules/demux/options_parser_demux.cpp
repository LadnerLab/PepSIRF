...
void options_parser_demux::parse( int argc, char ***argv, options *opts )
{
    ...
    desc.add_options()
        ...
        ( "input_r2", po::value<std::string>( &opts_demux->input_r2_fname )->required(), "Input reverse reads fastq file to parse.")
        ( "library,l", po::value<std::string>( &opts_demux->library_fname )->required(), "Designed library containing amino acid peptides. "
