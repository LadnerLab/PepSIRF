...
def main():
	...
	# Required arguments
	parser.add_argument("-i", "--input-dir", type=str, required=True, help="Directory containing a merge multiprotein cluster sequence file corresponding to each network. Can be generated using bulkClustering.py")
	parser.add_argument("-m", "--metadata", type=str, required=True, help="Tab-delimited metadata file which contains taxomic info")
	parser.add_argument("-o", "--output-dir", type=str, required=True, help="Directory to save taxomic summary of each network. This directory will be created, if it doesn't already exist.")
...
def gen_sum(
	...
	for file in glob.glob(os.path.join(input_dir, 'network_*_merged_multiprotein_cluster_sequences.tsv')):
		...
		# get data for output
		outDf = pd.DataFrame([x for x in sorted(clust_seqs_dict.keys(), key=lambda key: key)], columns=["MultiProteinCluster"])
		for col in tax_cols:
			col_out_data = list()
			for clust in clust_seqs_dict.keys():
				...
				for name in col_names:
					prop_list.append(col_counts[name] / total_count)
