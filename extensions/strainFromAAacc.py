...
def main():
	...
	reqArgs.add_argument('--outName',  help='Base filename to write Strain info, linked to the aa accessions.', required=True)
	...
	parser.add_argument('--rerunEmpty', default=False, action="store_true", help="Use this flag if you want the script to try again to download the xml in cases where previous attempts have resulted in an empty file.")
	...
	df = pd.read_csv(opts.data, sep="\t", header=0, keep_default_na=False, low_memory=False)
	...
	#Open a file to use as an error log, if file already exists, it will be opened in append mode
	if os.path.isfile(f"{opts.outDir}/{opts.errorLog}"):
		ferr = open(f"{opts.outDir}/{opts.errorLog}", "a")
	else:
		ferr = open(f"{opts.outDir}/{opts.errorLog}", "w")
		ferr.write("Command\tReturnCode\n")
	
	#Open a file to keep track of the samples with empty result, if file already exists, it will be opened in append mode
	if os.path.isfile(f"{opts.outDir}/{opts.emptyLog}"):
		femp = open(f"{opts.outDir}/{opts.emptyLog}", "a")
	else:
		femp = open(f"{opts.outDir}/{opts.emptyLog}", "w")
		femp.write("Command\n")
	
	#Open a file to keep track of the samples without an accession header in xml, if file already exists, it will be opened in append mode
	if os.path.isfile(f"{opts.outDir}/{opts.noLog}"):
		fnoac = open(f"{opts.outDir}/{opts.noLog}", "a")
	else:
				...
				for acc in accRaw.split(","):
					if "|" not in acc:
						...
						if not os.path.isfile(outName):
							gofetch=True
						elif os.stat(outName).st_size == 0 and opts.rerunEmpty:
							gofetch=True
						else:
							gofetch=False
						...
						if os.stat(outName).st_size == 0:
							...
							isoL.append("")
							cloL.append("")
							# I used to remove these empty files, but I'm not doing that anymore. Instead, I'm keeping them as a reminder that this accession has been fetched previously
							#os.remove(outName)
						else:
							try:
