...
def main():
    ...
    concatemer_sample = df_2_sdict( samples_df )
    ...
    while r1_lines and i1_lines:
        for r1, i1 in zip( r1_lines, i1_lines ):
            concat = get_seq( r1, r1_idx ) + reverse( get_seq( i1, i1_idx ) )

            if concat not in concatemer_sample:
                concatemer_sample[ concat ] = [ 'No Sample', 0 ]
    ...
    fun = lambda x: '\t'.join( list( st( spl( x[ 0 ] ) + 
                                         x[1::]
                                       )
                                   )
                             )

    print( 'Forward Read\tReverse Read\tSample\tCount' )
...
def df_2_sdict( df ):
    ...
    for idx, row in df.iterrows():
        if not str( row[ 'Sample' ] ).lower() == 'nan':
            concat = row[ 'Forward Sequence' ] + row [ 'Reverse Sequence' ]
